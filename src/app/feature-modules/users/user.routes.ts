...
router.post("/add",upload.single('profile_picture') ,authorize('create-staff'),body(ZuserCreate)  ,async (req, res, next) => {
  try {
...
router.post ("/authorize",authorize('create'),async(req,res,next)=>{
  try{
    res.send();
