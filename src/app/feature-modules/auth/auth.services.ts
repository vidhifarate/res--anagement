...
import { generateAccessToken, generateRefreshToken, invalidateToken, verifyToken } from "../../../utilities/tokens.js";
...
export const login = async (credentials: Pick<User, "email" | "password" | "role">) => {
  try {
    ...
    const accessToken =await generateAccessToken(user.role,user.id);
    const refreshToken = await generateRefreshToken(user.role,user.id
    );
