...
const publicKey = fs.readFileSync("public.key",'utf8');
 ...
 export const generateAccessToken=async(role:string,id:string)=>{
  const token = await jwt.sign({version:currentVersion ,role,id},
    privateKey,
    {
      algorithm:"RS256",
      expiresIn:"1d"
 ...
 export const generateRefreshToken=async(role:string,id:string)=>{
   const token = await jwt.sign({version:currentVersion ,role,id},
    privateKey,
    {
      algorithm:"RS256",
